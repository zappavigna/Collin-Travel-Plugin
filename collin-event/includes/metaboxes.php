...
function collin_event_add_all_metaboxes() {
    ...
    // MODIFICA: Cambiato il metabox da link a upload PDF
...
function collin_event_render_product_links_metabox( $post ) {
    ...
    ?>
    <p><label for="ticket_product_id"><strong>Ticket:</strong></label><br><select id="ticket_product_id" name="ticket_product_id" style="width:auto;"><option value="">-- Seleziona --</option><?php foreach($products as $p){printf('<option value="%s"%s>%s</option>',esc_attr($p->get_id()),selected($ticket_product_id,$p->get_id(),false),esc_html($p->get_name()));}?></select></p>
    <p><label for="shuttle_product_id"><strong>Navetta:</strong></label><br><select id="shuttle_product_id" name="shuttle_product_id" style="width:auto;"><option value="">-- Seleziona --</option><?php foreach($products as $p){printf('<option value="%s"%s>%s</option>',esc_attr($p->get_id()),selected($shuttle_product_id,$p->get_id(),false),esc_html($p->get_name()));}?></select></p>
    <p><label for="hotel_product_id"><strong>Hotel:</strong></label><br><select id="hotel_product_id" name="hotel_product_id" style="width:auto;"><option value="">-- Seleziona --</option><?php foreach($products as $p){printf('<option value="%s"%s>%s</option>',esc_attr($p->get_id()),selected($hotel_product_id,$p->get_id(),false),esc_html($p->get_name()));}?></select></p>
    <?php
}
...
// MODIFICA: Nuova funzione per il metabox del PDF
...
function collin_event_render_product_type_metabox( $post ) {
    ...
    ?><p><strong>Seleziona il tipo (per logica interna):</strong></p><?php foreach ( $options as $value => $label ) {?><p><label><input type="radio" name="ets_product_type" value="<?php echo esc_attr( $value ); ?>" <?php checked( $product_type, $value ); ?> /> <?php echo esc_html( $label ); ?></label></p><?php }
}
...
function collin_event_save_all_metabox_data( $post_id, $post ) {
    if ( !isset( $_POST['collin_event_metabox_nonce'] ) || !wp_verify_nonce( $_POST['collin_event_metabox_nonce'], 'collin_event_save_metaboxes' ) ) {
        if ($post->post_type == 'product' && current_user_can('edit_post', $post_id)) {
            if ( isset( $_POST['ets_product_type'] ) ) { update_post_meta( $post_id, '_ets_product_type', sanitize_text_field( $_POST['ets_product_type'] ) ); }
            if ( isset( $_POST['ets_linked_event_id'] ) ) { update_post_meta( $post_id, '_ets_linked_event_id', absint( $_POST['ets_linked_event_id'] ) ); }
        }
        return;
    }
    ...
    if ( $post->post_type == 'event' ) {
        ...
        // MODIFICA: Rimossa la vecchia riga di salvataggio del link lineup
        // if (isset($_POST['lineup_link_url'])) { update_post_meta($post_id, '_lineup_link_url', esc_url_raw($_POST['lineup_link_url'])); }
        
        // MODIFICA: Aggiunta la nuova riga per salvare l'ID del PDF
        ...
        // Salvataggio del campo URL Video
